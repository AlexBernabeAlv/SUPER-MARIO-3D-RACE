...
public class CreateThwomp : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (SceneManager.GetActiveScene().name == "LEVEL1")
        {
            ...
            for (int i = 0; i < 2; ++i)
            {
                if (i % 2 == 0) obj[i] = (GameObject)Instantiate(thwomp, new Vector3(-3.0f, 3.0f, 80.0f), thwomp.transform.rotation);
                else obj[i] = (GameObject)Instantiate(thwomp, new Vector3(3.0f, 3.0f, 80.0f), thwomp.transform.rotation);
    ...
    void Update()
    {
        for (int i = 0; i < obj.Length; ++i)
        {
            if (direction == 0)
            {
                ...
                {
                    obj[i].transform.Translate(-transform.up * fallSpeed * Time.deltaTime, Space.World);
                } else
                {
                    direction=1;
                }
            ...
            {
                if (obj[i].transform.position.y < 3)
                {
                    obj[i].transform.Translate(transform.up * fallSpeed / 2 * Time.deltaTime, Space.World);
                }
                else
                {
                    direction = 0;
